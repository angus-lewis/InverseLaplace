...
function ilt(fun, T, maxFnEvals, method)
    if method == "talbot" 
        ilt = talbot_inversion(fun,T,maxFnEvals)
    else
        # JLD2.@load pwd()*"/CMEParams.jld2" CMEParams
        ...
        ilt = 1 ./T .* sum(real(eta .* fun.(beta./T)));
    end

    return ilt 
end

function talbot_inversion(f_s, t, M)
